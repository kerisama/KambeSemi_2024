...
LED_BRIGHTNESS = 10
LED_INVERT = False
LED_PER_PANEL = 16  # 縦・横のLEDの個数

# Matrix setup
MATRIX_WIDTH = 2  # マトリクスLEDの枚数 (横)
MATRIX_HEIGHT = 1  # マトリクスLEDの枚数 (縦)
MATRIX_GLOBAL_WIDTH = MATRIX_WIDTH * LED_PER_PANEL  # 全体のLEDの数 (横)
MATRIX_GLOBAL_HEIGHT = MATRIX_HEIGHT * LED_PER_PANEL    # 全体のLEDの数 (縦)

# Communication setup
SLAVE_IPS = ['192.168.10.61']  # Add more as needed
PORT = 12345
...
def draw_frame(frame_pixels, color):
    ...
    for p in frame_pixels:
        ...
        if index >= 0 and index < len(slave_pixels):  # Ensure valid slave index
            slave_pixels[index].append((p[0], p[1]))
